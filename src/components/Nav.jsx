    ...
    <header className="sticky top-0 z-50 text-gray-700 backdrop-blur-[8px]">
      <div className="max-w-7xl mx-auto flex items-center justify-between px-4 h-16">
        ...
        <nav className="hidden md:flex gap-12 text-sm font-medium">
          ...
          {(user?.role == 'admin' || user?.role == 'superAdmin' || user?.role == 'inspector') && (<>
            <NavLink to="/admin/reports" className="hover:text-primary">View Reports</NavLink>
          </>)}

          {(user?.role == 'admin' || user?.role == 'superAdmin') && <NavLink to="/admin/logs" className="hover:text-primary"> Logs</NavLink>}
          {(user?.role == 'admin' || user?.role == 'superAdmin') && <NavLink to="/admin/supports" className="hover:text-primary"> Supports</NavLink>}
            ...
            user ?
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end" className="p-4">
                  ...
                  {user?.role == 'superAdmin' && <NavLink to="/admin/settings"> <DropdownMenuItem>Settings</DropdownMenuItem> </NavLink>}
